function wp_action(data, svg_area, silent) {
    ...
    if (total_edits == 1) {
        $('#edit_counter').html('You have seen <span>' + total_edits + ' Payment</span>.');
    } else {
        $('#edit_counter').html('You have seen a total of <span>' + insert_comma(total_edits) + ' Payments</span>.');
    }
    var now = new Date();
    edit_times.push(now);
    to_save = [];
...
wikipediaSocket.init = function(ws_url, lid, svg_area) {
    ...
    channel.bind('my_event', function(resp) {
      console.log("Message Received");
                ...
                data.change_size = data.pAmount;
                data.label_text = data.pName;
                data.url = "http://localhost:3000";
                data.ns = "Main";
                data.page_title = data.pName;
                ...
                if (data.ns == 'Main' || DEBUG) {
                    if (!isNaN(data.change_size) && (TAG_FILTERS.length == 0 || $(TAG_FILTERS).filter($.map(data.hashtags, function(i) { 
                        ...
                        if (data.change_size < 0) {
                            ...
                            if (data.change_size == 1) {
                                rc_str += ' paid ' + Math.abs(data.change_size);
                            } else {
                                rc_str += ' paid ' + Math.abs(data.change_size);
                            }
                        }
...
function update_epm(epm, svg_area) {
    if (!epm_text) {
        ...
        epm_text = epm_container.append('text')
            ...
            .style('font-size', '.8em')
            .text(epm + ' Payments per minute');

    } else if (epm_text.text) {
        epm_text.text(epm + ' Payments per minute');
    }
}
